...
predicted_counts = np.load("data/predicted_counts_arr.npy")
...
plt.imshow(predicted_counts, cmap="viridis", origin="lower", aspect="auto")
...
for i in range(len(delta_m14_squared)):
    for j in range(len(sin_squared_2_theta_14)):
        text = plt.text(j, i, np.round(predicted_counts[i, j], 1),
